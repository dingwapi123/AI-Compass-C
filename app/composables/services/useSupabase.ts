...
export function useSupabaseRest() {
   ...
   */
  async function get<T = unknown>(path: string, params?: Record<string, unknown>, options?: FetchOptions<'json'>): Promise<T> {
   ...
   */
  async function getRaw<T = unknown>(path: string, params?: Record<string, unknown>, options?: FetchOptions<'json'>): Promise<{ data: T, headers: Headers }> {
    const url = `${base}/rest/v1/${path}`
