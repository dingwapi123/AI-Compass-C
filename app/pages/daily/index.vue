    ...
    <UContainer>
      <div class="flex flex-col gap-6 py-10 md:flex-row md:items-center md:justify-between">
        <div class="space-y-3 max-w-2xl">
          ...
          <p class="text-neutral-600 dark:text-neutral-300">汇总过去几日的重要更新，按来源、标签与关键词快速检索。</p>
        </div>
        <div class="w-full max-w-lg">
          <div class="rounded-2xl border border-neutral-200/70 bg-white/80 p-4 shadow-sm backdrop-blur dark:border-neutral-800 dark:bg-neutral-900/80">
            <div class="text-sm font-semibold text-neutral-700 dark:text-neutral-200 mb-3">筛选</div>
      ...
      <div v-if="groupedNews.length" class="space-y-8">
        <div v-for="group in groupedNews" :key="group.date" class="space-y-3">
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <UCard v-for="item in group.items" :key="item.id" class="hover:-translate-y-1 hover:shadow-lg transition">
              <template #header>
              ...
              <p class="text-sm text-neutral-600 dark:text-neutral-300 line-clamp-3">{{ item.summary }}</p>
              <div class="mt-3 flex flex-wrap items-center gap-2 text-xs text-neutral-500">
                ...
                <UButton :to="item.source_url" target="_blank" variant="ghost" size="2xs" icon="i-lucide-external-link">源站</UButton>
              </div>
            </UCard>
          </div>
        </div>
...
const sourceOptions = computed(() => {
  ...
  return [{ label: '全部来源', value: undefined }, ...opts.map((s) => ({ label: s, value: s }))] as const
})
...
const filtered = computed<NewsItem[]>(() => {
  return mockNews.filter((item) => {
    ...
    const kwOk = kw
      ...
      : true
    const sourceOk = selectedSource.value ? item.source === selectedSource.value : true
...
const groupedNews = computed(() => {
  ...
  return Array.from(map.entries())
    .map(([date, items]) => ({ date, items: items.sort((a, b) => new Date(b.published_at).getTime() - new Date(a.published_at).getTime()) }))
